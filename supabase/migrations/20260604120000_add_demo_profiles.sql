-- Add missing demo profiles for existing demo auth users
-- This migration is idempotent and will only insert missing profile rows.

-- Create profile rows for demo auth users if missing
WITH demo_users AS (
  SELECT id, email
  FROM auth.users
  WHERE email IN (
...
-- Create chef_profiles rows for demo chef accounts if missing
WITH demo_chefs AS (
  SELECT id, email
  FROM auth.users
  WHERE email IN ('chef@chefbid.com', 'chef_pending@chefbid.com')
)
...
FROM demo_chefs c
WHERE NOT EXISTS (
  SELECT 1 FROM chef_profiles cp WHERE cp.user_id = c.id
);
